...
# setup locust stack via cloudformation as we can load large files onto the instances.
resource "aws_cloudformation_stack" "master" {
  ...
  parameters {
      ...
      Name = "${var.name}"
      SupervisorConf = "${replace(file("./tfmodules/nodes/supervisord.conf"), "$${var.locust_command}", "${var.locust_command}")}"
  }
  template_body = "${file("./tfmodules/nodes/cloudformation.json")}"
}
